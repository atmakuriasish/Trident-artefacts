...
def process_perf_log_new(path, measurements):
    ...
    if 'vmstat' in path:
        ...
        for line in fd:
            ...
            # Clean and parse the line using regex
            cleaned_line = line.replace(",", "").replace("-", "_")
            match = re.match(r"\s*(\d+)\s+([\w\.:]+)", cleaned_line)

            if match:
                value = int(match.group(1))
                raw_metric = match.group(2)
                #print(raw_metric)
                
                # Transform metric names to match your initialized structure
                metric_name = raw_metric.replace(":u", "")\
                                        .replace(".store_", "_store_")\
                                        .replace(".load_", "_load_")\
                                        .replace("dtlb_", "dtlb_")\
                                        .replace("page_walker", "page_walker")  # 

                # Special case for cycles:u → cycles
                if metric_name == "cycles":
                    metric_name = "cycles"
                elif metric_name == 'page_faults':
                    metric_name="pagefaults"
                elif metric_name == 'dTLB_loads':
                    metric_name="dTLBloads"
                elif metric_name == 'dTLB_stores':
                    metric_name="dTLBstores"
                elif metric_name == 'L1_dcache_load_misses':
                    metric_name="L1dcacheloadmisses"
                elif metric_name == 'L1_dcache_loads':
                    metric_name="L1dcacheloads"
                elif metric_name == 'L1_dcache_stores':
                    metric_name="L1dcachestores"
                elif metric_name == 'LLC_load_misses':
                    metric_name="LLCloadmisses"
                elif metric_name == 'LLC_loads':
                    metric_name="LLCloads"
                elif metric_name == 'LLC_store_misses':
                    metric_name="LLCstoremisses"
                elif metric_name == 'LLC_stores':
                    metric_name="LLCstores"

                # Only store metrics that exist in your initialization
                #if metric_name in metric_vals:
                metric_vals[metric_name] = value
                #print(metric_name+":"+str(metric_vals[metric_name]))
